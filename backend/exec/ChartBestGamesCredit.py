...
#         "$lookup" : {
#       "from" : 'games',
#       "localField" : 'internal.id',
#       "foreignField" : 'data.game_id',
#       "as" : 'game_data'
#   } 
#   },
...
for tmp in list(xtest):
    ...
    if xCreditAgg:
        ...
        for tmp in list(xCreditAgg):
            ReturnAmount.append({'amount' : tmp["amount"] , "game" : gameId , "name" : nameGame})
...
for tmp in list(ReturnGamesId):
    ...
    if xBrandTemp:
        for tmp2 in list(xBrandTemp):
            BrandsTemp.append({"brand" : tmp2['internal']['provider_name'] , "game" : tmp })
            BrandsClear.append({"brand" : tmp2['internal']['provider_name'] , "credit" : 0 })
 
BrandsClear = { each['brand'] : each for each in BrandsClear }.values()

# pprint(list(BrandsClear))

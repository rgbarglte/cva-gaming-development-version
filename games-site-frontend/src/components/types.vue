<template>
    ...
    <el-scrollbar max-height="60px" class="hide-desktop-types show-mobil-types">
        <div class="scrollbar-flex-content">
            <a v-for="item in types" key="item" v-if="loading.types == false" :href="'/type/' + item.slug"
                class="scrollbar-demo-item ">
                <div
                    style="height: 30px;width:30px;border-radius:100%;background:rgb(255,255,255,0.1);margin-right: 10px;">
                </div> {{ item.name }}
            </a>
        </div>
    </el-scrollbar>
